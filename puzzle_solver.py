"""CS 205 8-puzzle solver.

The same search loop is used for Uniform Cost Search and the two A* heuristics.
The priority of a node is f(n) = g(n) + h(n).
"""
...
# Save each goal coordinate once, so Manhattan distance does not have to search
# the goal board every time it checks a tile.
GOAL_POSITIONS = {
...
class SearchNode:
    """A board in the search tree, plus enough history to print the answer."""
...
def get_custom_puzzle():
    ...
    while True:
        ...
        # Check the easy input mistakes first, then check whether the puzzle can
        # actually be solved.
...
def general_search(initial_state, heuristic_function, show_trace=False):
    """The shared search loop for Uniform Cost Search and A*."""
    ...
    # best_g_cost_by_state lets the program avoid keeping a worse path to a
    # board it has already reached.
    best_g_cost_by_state = {puzzle_to_tuple(initial_state): 0}
    explored_states = set()
    nodes_expanded = 0
    ...
    while nodes["heap"]:
        ...
        # The heuristic function is passed in, so this one loop can run all
        # three algorithms.
...
def add_node_to_queue(nodes, node):
    # heapq needs a full tuple ordering. The counter handles exact ties without
    # asking Python to compare SearchNode objects.
    heapq.heappush(
        nodes["heap"],
...
def uniform_cost_heuristic(puzzle):
    # Uniform Cost Search is just A* with no estimate of the remaining cost.
    return 0


def misplaced_tile_heuristic(puzzle):
    # Count tiles that are not home yet. The blank does not count as a tile.
    misplaced_tiles = 0
...
def manhattan_distance_heuristic(puzzle):
    # Add how many rows and columns each tile is away from its goal square.
    total_distance = 0
    ...
    for row_index in range(PUZZLE_SIZE):
        for column_index in range(PUZZLE_SIZE):
            ...
            total_distance += abs(row_index - goal_row)
            total_distance += abs(column_index - goal_column)
...
def expand_puzzle(puzzle):
    ...
    for move_name, row_change, column_change in OPERATORS:
        # Illegal moves return None, so only real moves become children.
...
def move_blank_tile(puzzle, row_change, column_change):
    ...
    # Work on a copy so the parent board stays unchanged.
...
def is_solvable_puzzle(puzzle):
    # For odd-width boards like the 8-puzzle, an even inversion count means the
    # board is reachable from the goal.
...
def puzzle_to_tuple(puzzle):
    # Tuples can be used in sets and dictionaries; lists cannot.
